...
public class SysCommonApiImpl extends BaseService implements SysCommonApi {
    ...
    public SysAppVersion validateVersion(SysAppVersion appVersion) throws RpcException {
        try {
            ...
            condtion.put("merchantId", appVersion.getMerchantId());
    ...
    @Override
    public SysAbout getSysAboutByType(Integer merchantId, byte appType) throws RpcException {
        try {
        	Map<String, Object> params = Maps.newHashMap();
        	params.put("merchantId", merchantId);
        	params.put("appType", appType);
        	// params.put("delFlag", Constant.DEL_FLAG_0);
        	List<SysAbout> list = sysAboutService.findByBiz(params);
    ...
    @Override
    public PaginationDto<MessageMailVo> getMessageMailByPage(Integer merchantId, Integer userId, String type, Byte appType, SimplePageDto pageDto) throws RpcException {
    ...
    @Override
    public List<SysCarouselConf> getCarouselList(Integer merchantId, String carouselType) throws RpcException {
        try {
        	Map<String, Object> params = Maps.newHashMap();
        	params.put("merchantId", merchantId);
        	params.put("carouselType", carouselType);
        	params.put("delFlag", Constant.DEL_FLAG_0);
        	return carouselService.findByBiz(params);
        } catch (ServiceException e) {
	...
	@Override
    public boolean sendAppletMessage(Integer merchantId, WxMaTemplateMessage vo) throws RpcException {
    	Assert.notNull(vo, "消息对象不能为空");
    	try {
    		WeixinMaService wxService = wxInstanceFactory.getMiniAppInstance(merchantId);
    		WxMaMsgService msgService = wxService.getMsgService();
    		msgService.sendTemplateMsg(vo); 
			return true;
		} catch (WxErrorException e) {
			logger.error("发送小程序模板消息失败", e);
            throw new RpcException("", "发送小程序模板消息失败", e);
		}    		
    }
    
	@Override
	public boolean sendWxTemplateMessage(Integer merchantId, WxMpTemplateMessage vo) throws RpcException {
		try {
			...
			String msgid = msgService.sendTemplateMsg(vo);
			return StringUtils.isNotBlank(msgid) ? true : false;
		} catch (WxErrorException e) {
			logger.error("发送微信公众号模板消息失败", e);
			throw new RpcException("", "发送微信公众号模板消息失败", e);
		}
	}
