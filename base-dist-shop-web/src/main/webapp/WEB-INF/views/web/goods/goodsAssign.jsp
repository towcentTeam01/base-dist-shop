...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active">
		  <a href="${ctx}/goods/goodsChannelDtl?channelId=${channel.id}&assigned=${channelGoods.assigned}">频道商品分配</a>
		</li>
	</ul>
	...
	<form:form id="searchForm" modelAttribute="channelGoods" action="${ctx}/goods/goodsChannelDtl" method="post"
		...
		<ul class="ul-form">
			<li>
			    <label>商品分类：</label> 
			    <sys:treeselect id="goodsCategory"
					name="goodsCategoryId" value="${channelGoods.goodsCategoryId}"
					labelName="goodCategoryName"
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<tbody>
			<c:forEach items="${page.list}" var="channelGoods">
				<tr>
					<td>
					    <input class="cb-goosChannel-${channelGoods.assigned}" value="${channelGoods.goodsId}" type="checkbox"
						<c:if test="${channelGoods.assigned==1}">disabled="disabled"</c:if>>
				    </td>
					<td>
					    <img style="width: 50px; height: 50px; object-fit: contain; background-color: #e5e1e1;"
						src="${channelGoods.goodsSmallPic}" />
					</td>
